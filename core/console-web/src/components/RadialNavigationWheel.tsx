...
export const RadialNavigationWheel: React.FC<RadialNavigationWheelProps> = ({
  ...
  // Weiträumiger, großzügiger Radius (380px) für maximale Raumausnutzung
  const radius = 380;

  return (
    <div className="relative flex flex-col items-center justify-center min-h-[840px] w-full py-12 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950 rounded-3xl border border-cyan-500/25 shadow-[0_0_80px_rgba(6,182,212,0.18)] overflow-hidden">
      {/* 1. ROTIERENDE & PULSIERENDE SVG ORBIT-RINGE (WEITRÄUMIG) */}
      <svg
        className="absolute inset-0 w-full h-full pointer-events-none z-0 overflow-visible"
        viewBox="-500 -500 1000 1000"
      >
        <defs>
          <radialGradient id="cyberCenterGlowExpansive" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#06b6d4" stopOpacity="0.3" />
            <stop offset="50%" stopColor="#3b82f6" stopOpacity="0.12" />
            <stop offset="100%" stopColor="#0f172a" stopOpacity="0" />
          </radialGradient>

          <linearGradient id="neonArcGradientExpansive" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#00f2fe" stopOpacity="0.95" />
            <stop offset="50%" stopColor="#3b82f6" stopOpacity="0.75" />
            <stop offset="100%" stopColor="#a855f7" stopOpacity="0.95" />
          </linearGradient>

          <filter id="neonGlowFilterExpansive" x="-30%" y="-30%" width="160%" height="160%">
        ...
        {/* Center Glow Aura */}
        <circle cx="0" cy="0" r={radius + 80} fill="url(#cyberCenterGlowExpansive)" />
        ...
        <circle
          ...
          r={radius + 25}
          fill="none"
          stroke="url(#neonArcGradientExpansive)"
          strokeWidth="2"
        ...
        <circle
          ...
          stroke="url(#neonArcGradientExpansive)"
          strokeWidth="2.5"
          filter="url(#neonGlowFilterExpansive)"
          ...
          return (
            <g key={`ray-${agent.id}`}>
              <line
                ...
                filter={isSelected || isSuggested ? "url(#neonGlowFilterExpansive)" : undefined}
      ...
      {/* 2. ZENTRALES SUCHFELD HUB (Im Zentrum des großzügigen Rads) */}
      <div className="relative z-20 w-full max-w-2xl px-6">{children}</div>

      {/* 3. WEITRÄUMIGE ORBITAL FACHAGENTEN NODES */}
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Node Title Label */}
                <span className="text-xs sm:text-sm font-black mt-3 whitespace-nowrap tracking-wider uppercase text-slate-100 group-hover:text-cyan-300 transition-colors">
                  {agent.name}
                </span>
                ...
                {isHovered && (
                  <div className="absolute top-full mt-4 left-1/2 -translate-x-1/2 w-60 p-3.5 bg-slate-950/95 text-slate-100 rounded-2xl text-xs font-sans shadow-[0_15px_50px_rgba(0,0,0,0.9)] z-50 pointer-events-none border border-cyan-500/50 backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-150">
                    <div className="flex items-center justify-between font-extrabold text-cyan-400 uppercase tracking-wider text-[11px] border-b border-slate-800 pb-1 mb-1">
