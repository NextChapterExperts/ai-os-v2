...
- **Status (2026-08-12):** ✅ Implementiert als SDK-Agent (`agents/research/agent.py`) + Orchestrator Intent Handler (`core/orchestrator/handlers/research_handler.py`). Pop-up Modal Workspace (`ResearchAgentModal.tsx`) unter Verfügbare Agenten / `/workflows/research` integriert mit:
  1. Fullscreen Pop-up Modal-Arbeitsfenster für Recherchen.
  2. Automatischer Persistierung / Speicherung von Recherche-Ergebnissen im Unternehmensgedächtnis (`org:KnowledgeAsset` in Company Brain / `kg_nodes`).
  3. Säuberung von HTML/Script-Code aus Quelltext-Snippets (clean text extraction).
  4. Konsistente Modellauswahl gemäß Lagebild (`compute.yaml`), inklusive lokaler Sovereign Monster-Modelle & OpenRouter Cloud-Optionen.
  - Release-Tag: `roadmap/2026-08-12-p4-research-agent-modal-save` (22/22 Tests PASS).
...
- **Console Lagebild & Run Context Fixes:** Reaktionsschlüssel-Fix in `RunContextPanel.tsx` (`${chunk.id ?? 'chunk'}-${idx}`), Inline-Button für Kontext-Inspektion direkt unter der KI-Zusammenfassung in `MemorySearch.tsx` wiederhergestellt.
- **Orchestrator Run Context Speicherung:** `import asyncio` + `datetime` ergänzt und garantierte Zustandsspeicherung unter `/opt/ai-os/memory/state/run-context/{run_id}.json` für jeden Orchestrator-Dispatch-Run in `server.py` verankert.
- **LLM Answer Synthese für Unified Search:** Unified Search synthetisiert Top-Hits nun per LLM zu einer natürlichen Antwort (`kind: "ask"`), wodurch leere Quelllisten vermieden werden.
- **Chat Re-Ingestion & Vektor-Score Ranking:** Alle 31 Antigravity- & Cursor-Chat-Protokolle (~2.000 Nachrichten) per `antigravity-job.mjs` neu in L1/L2 Memory ingestiert. In `memory_ask.py` die Relevanzsortierung auf echten Vektor-Score (`score`) umgestellt, damit Wissensfragen nicht mehr von repetitivem Graph-Metadaten-Rauschen verdrängt werden.
- **Knowledge Finder & Suche UI Fix:** Frontend-State-Parsing in `UnifiedSearch.tsx` gefixt (`json.sources ?? json.result?.sources`), sodass die Seite `/search` verlässlich alle Treffer aus Graph, Freigegeben, Rohdateien und Cursor-Chats anzeigt. KI-Zusammenfassungs-Karte oben auf der Suchseite ergänzt und UI-Rollen zwischen Lagebild (KI-Assistent) und Suche (Knowledge Finder Index) geschärft.
- **Inferenz-Gateway Speedup:** Timeout für vLLM auf 1.0s gesenkt, sodass Fallbacks auf Ollama LAN (`qwen2.5-coder:14b`) ohne spürbare Verzögerung (~1.5s Antwortzeit) durchlaufen.

## Stand & Changelog (2026-08-10 — Inferenz & LiteLLM Ollama IP Sync)

- **Inferenz / LiteLLM & Ollama IP-Fix (§23):** `OLLAMA_HOST` Standard auf `192.168.178.116` in `.env`, `.env.example`, `deploy/infra.yml`, `deploy/core.yml`, `appliance/config/defaults/.env.template` und `docs/04-DEPLOYMENT.md` aktualisiert.
...
- **Maximale Bildschirmausnutzung (`RadialNavigationWheel.tsx`):**
  - Riesiger Rad-Radius (**520px**) mit `min-h-[1050px]` für volle Viewport-Dominanz.
  - Große Icon-Knoten (40px Icons) und weiterentwickelte Cyber-Laser-Bahnen.
- **Riesige Webrecherche-Schaltzentrale (`prototype/page.tsx`):**
  - Vergrößerung des Suchfensters auf **`max-w-3xl`** (850px Breite) mit großzügigem Eingabefeld (`py-5 text-base`).
  - Vollwertige Webrecherche-Funktion mit SearXNG-Egress, Quellennachweisen und Synthese-Ansicht bei Suchen.
- **Master Testsuite:** 100% grün (0 Failed).

---

## Stand & Changelog (2026-08-13 — Großzügiges Raumausfüllendes Navigationsrad)

- **Release-Tag:** `roadmap/2026-08-13-p4-expansive-radial-wheel`
- **Raumausnutzung & Weite (`RadialNavigationWheel.tsx`):**
  - Erweiterung des Rad-Radius auf **380px** mit großzügigen Knotenabständen für volle Bildschirm-Präsenz.
  - Breiteres, elegantes Pill-Suchfeld (`max-w-2xl`) im Zentrum des Rads.
  - Reines Navigationsrad-Interface ohne jegliche Unter-Boxen.
- **Master Testsuite:** 100% grün (0 Failed).

---

## Stand & Changelog (2026-08-13 — Harmonisierte Puristische Startseite ohne Unter-Boxen)

- **Release-Tag:** `roadmap/2026-08-13-p4-pure-harmonious-radial-page`
- **Perfekte Harmonie & Raum-Proportionen (`RadialNavigationWheel.tsx`):**
  - Feinjustierung des Rad-Radius auf perfekt ausbalancierte **280px** mit harmonisch zentrierter Suchschaltzentrale.
  - Elegante Hover-Erklärungs-Tooltips ohne Störung des visuellen Gleichgewichts.
- **100% Puristische Startseite (`prototype/page.tsx`):**
  - **Vollständige Entfernung aller zusätzlichen Boxen unterhalb des Rads.**
  - Klick auf Fachagenten führt zum direkten Navigations-Absprung (z. B. `/agents?agent=handwerker`), sodass die Startseite absolut sauber und fokussiert bleibt.
- **Master Testsuite:** 100% grün (0 Failed).

---

## Stand & Changelog (2026-08-13 — Full-Width Screen Layout & Klick-Absprung Navigationsrad)

- **Release-Tag:** `roadmap/2026-08-13-p4-radial-wheel-fullwidth`
- **Full-Width Viewport (`AppShell.tsx` & `prototype/page.tsx`):**
  - Aufhebung der `max-w-6xl` Begrenzung für volle Bildschirmausnutzung über die gesamte Browserbreite.
  - Vergrößerung des Rad-Radius auf **440px** für imposante Darstellung auf allen Desktop-Monitoren.
- **Interaktionsmodell:**
  - Hovern zeigt eine elegante Erklärungskarte (Tooltip) zum Fachagenten.
  - Erst ein expliziter **Klick** führt zum Absprung in den Fachagenten-Arbeitsbereich (`/agents?agent=<id>`).
- **Master Testsuite:** 100% grün (0 Failed).
