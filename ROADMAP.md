...
**Stand:** August 2026 (aktualisiert 2026-08-12 — Recherche-Agent Prototyp & Cockpit UI: SDK-konformer `ResearchAgent` mit Dual-Retrieval (Company Brain + SearXNG Web-Suche), Anonymitäts-Metadaten, interaktivem Dashboard (`/workflows/research`), Modell-Auswahl-Button, Prompt-Inspektor-Modal („Kompletten Prompt anzeigen“) & 20 Randbedingungen-Testfällen (`tests/test_research_agent.py`); zuvor 2026-08-11 — Company Brain Synthese, Knowledge Finder & Run Context UI Restauration: Lagebild LLM-Zusammenfassung & Kontext-Inspektor wiederhergestellt, Orchestrator Run Context Speicherung in `/opt/ai-os/memory/state/run-context/` garantiert, Vektor-Score Sortierung in `memory_ask.py` vor Quelltyp-Priorität gestellt, Knowledge Finder (`/search`) JSON-Response Parsing gefixt & KI-Zusammenfassungs-Karte oben ergänzt; 31 Antigravity & Cursor Chats re-ingestiert; zuvor 2026-08-10 — Sovereign-Speed Optimization (14B Default): Default-Sovereign & Coder Modell auf `qwen2.5-coder:14b` umgestellt, wodurch Inferenz-Antwortzeiten von 30s+ Timeout (32B VRAM-Offloading) auf ~6.9s sinken; vLLM Modell-Formatierung korrigiert (170ms High-Throughput); zuvor 2026-08-10 — Console Lagebild UI Dropdown: Modell-Auswahl in `ComputeModePanel.tsx` von unübersichtlichen Buttons auf elegantes `<select>`-Dropdown-Menü mit Sofort-Umschaltung & Detail-Anzeige umgestellt; zuvor 2026-08-10 — Inferenz-Performance & GPU-VRAM Keep-Alive: `keep_alive: 30m` in `_call_ollama_direct` gesetzt & `think` Param gereinigt, wodurch Inferenz-Antwortzeiten von ~24s (Kaltstart/VRAM-Swap) auf 0.94s (VRAM-Warm-Hit) sinken; zuvor 2026-08-10 — Inferenz & LiteLLM Ollama IP Sync: `OLLAMA_HOST` Standard auf `192.168.178.116` in .env, Compose-Infra/Core, Appliance & Doku gesetzt, damit LiteLLM & Memory Gateway das erreichbare KI-Server Monster-Modell ansteuern; zuvor 2026-08-10 — Smart Auto-Router & Manual Model Override: Intent-basiertes automatisches Modell-Routing `auto` als Default-Modus + manuelles Modell-Pinning im Lagebild UI; zuvor 2026-08-10 — KI-Server Monster-Modell-Integration: Ollama IP 192.168.178.116, Qwen2.5-Coder 32B, DeepSeek-R1 32B, Mistral-Nemo 12B, Hermes-3 8B, Llama3.2-Vision 11B & vLLM Qwen2.5-14B in Compute-Modi, Model-Gateway & Lagebild UI; zuvor 2026-08-08 — Local Model Gateway Fix & 60s Ollama Timeout, Context Size Badge & LLM Context Link Restoration, CAP & Dashboard Project Memory Search Enhancement; zuvor 2026-08-04 — Portfolio & Lagebild auf `studentenprojekt_v2` / 4-Säulen-Sprint; zuvor 2026-08-03 — Email-Fachagent & Release-Tags; zuvor 2026-07-30 — Universal File Upload Ingest Service & Drag-Drop UI, Appliance VM Packaging Automation & Cloud-Init Bootstrap, Agent SDK Contract Framework, Deterministic Generic Workflow Engine, DataProduct Schema Export APIs & Dynamic Console UI Form/Result Renderers, Compute Mode Fallback & Service Health Regression Suite, Daily Open Loops Formatting Cleanup & Navigation Context Preservation, Daily Open Loops Clarity & Direct Navigation Links, Synthesized Meeting Summary Restoration, Regression Test Suite for Meeting Search & Dispatch, Orchestrator Live Reload & Compute Mode Fallback, Meeting To-Do Search Integration, Wyrd-Key & Odin Legend Popover, Lagebild UI Cleanup & Prompt Dropdown, Standard Prompt Catalog & Refined Intent Router, VIRKI UI Branding, Git Push Automation & Multi-Repo Sync, Document Viewers, WAQAM & Studentenprojekt Restrukturierung, Projekt Zorro)  Zorro)ekt Restrukturierung, Projekt Zorro)  
